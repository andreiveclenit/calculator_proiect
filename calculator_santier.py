...
with st.sidebar:
    ...
    st.markdown("### 🚌 Transport & Logistică")
...
working_days_count = 0 # Zile efective de lucru (pentru PM/TPL)
calendar_data = []
...
for day in days:
    ...
    # 3. ZILE DE LUCRU (ȘANTIER)
        ...
        if wd < 5: # Luni-Vineri
            ...
            hours_paid_work = 10
            working_days_count += 1 # Contabilizăm zi lucrătoare pentru PM/TPL
        elif wd == 5: # Sâmbătă
...
# --- CALCUL COSTURI NOI (PM, TPL, Consumabile, Argon) ---
...
col_work, col_transp, col_others = st.columns(3)

with col_work:
    st.subheader("🛠️ Manoperă (Șantier)")
    st.caption("Doar orele lucrate pe șantier")
...
with col_transp:
    st.subheader("🚛 Transport & Drum")
    st.caption("Plată ore drum + Combustibil")
    ...
    st.markdown(f"**Subtotal:** :orange[{grand_total_transport:,.2f} €]")

with col_others:
    st.subheader("📋 Alte Costuri")
    st.caption("PM, TPL, Consumabile")
    ...
    st.text_input(f"Consumabile ({consumables_pct}%)", value=f"{cost_consumables:,.2f}", disabled=True, help="% din Manopera Muncitori")
